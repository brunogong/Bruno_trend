...
# --- FUNZIONE DI RECUPERO DATI (AUTO-SCAN) ---
def get_massive_data(symbol):
    # Formattazione Ticker v3: C:EURUSD o X:XAUUSD
    ticker = f"X:{symbol}" if "XAU" in symbol else f"C:{symbol}"
    
    # Lista di possibili endpoint v2/v3 per i prezzi real-time
    endpoints = [
        f"https://api.massive.com/v2/last/forex/{ticker}",
        f"https://api.massive.com/v2/last/nbbo/{ticker}",
    ...
    for url in endpoints:
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                # Cerchiamo il prezzo nel campo 'last' o 'results'
...
st.subheader("🎯 Segnali Live (Aggancio API)")
rows = []

for asset in assets:
    data = get_massive_data(asset)
    if data:
        # Estrazione flessibile del prezzo (p=price o a=ask)
...
if rows:
    ...
    st.warning("⚠️ Collegamento stabilito. Se i 404 persistono, controlla nel tab 'REST API Quickstart' su Massive l'URL dell'esempio 'Last Quote'.")

# --- GRAFICO ---
st.markdown("---")
st.subheader("📈 Analisi Tecnica Candlestick")
sel_asset = st.selectbox("Asset Selezionato:", assets)
...
fig.update_layout(template="plotly_dark", xaxis_rangeslider_visible=False, height=550, margin=dict(t=30, b=10))
